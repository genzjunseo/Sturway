...
token = 'NzkxNTM5MTY3OTk3MzI5NDE4.X-QoSw.eC-_ob3CR1aq77Qhn3FcNtoGXSw'
...
async def on_ready():
    ...
    while True:
       ...
       messages = ["안녕하세요!", "Sturway#2482님이 제작하신 봇" , str(user) + "명이 저와 놀고있어요!", str(server) + "개의 서버에서 안전하게 보관되고 있어요!","!도움으로 저의 명령어를 알아보세요!"]
       for (m) in range(5):
...
async def on_message(message):
    ...
    if message.content.startswith("!밴"):
        if message.author.guild_permissions.administrator:
            ...
            await message.channel.send(f"관리자 권한이 필요합니다")
    ...
    if message.content == ("!초대"):
        await message.channel.send ("https://discord.com/api/oauth2/authorize?client_id=791539167997329418&permissions=0&scope=bot")
